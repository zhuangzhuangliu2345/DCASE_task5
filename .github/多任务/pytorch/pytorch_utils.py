...
def forward(model, generate_func, cuda, return_input=False, 
    ...
    for batch_data_dict in generate_func:
        ...
        batch_spacetime = move_data_to_gpu(batch_data_dict['spacetime'], cuda)
        with torch.no_grad():
            model.eval()
            batch_output1 = model.forward1(batch_feature, batch_spacetime)
            batch_output2 = model.forward2(batch_feature, batch_spacetime)

        append_to_dict(dict=output_dict, key='audio_name', 
            value=batch_data_dict['audio_name'])
        
        append_to_dict(dict=output_dict, key='output1',
            value=batch_output1.data.cpu().numpy())
        append_to_dict(dict=output_dict, key='output2',
                       value=batch_output2.data.cpu().numpy())
