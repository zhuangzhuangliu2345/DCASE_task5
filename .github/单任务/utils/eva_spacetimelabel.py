...
annotation_path = '/home/fangjunyan/2020shuju/annotations.csv'
...
def get_subset_split(annotation_data):
    ...
    return np.array(train_idxs), np.array(valid_idxs), np.array(eva_idxs)
...
train_idxs, valid_idxs, eva_idxs= get_subset_split(annotation_data)
...
X_valid_loc = np.array([[latitude_list[idx],
                             longitude_list[idx]]
                            for idx in valid_idxs])
X_eva_loc = np.array([[latitude_list[idx],
                             longitude_list[idx]]
                            for idx in eva_idxs])
...
X_eva_spacetime = np.concatenate((X_eva_time, X_eva_loc), axis=-1)
...
hf3 = h5py.File(os.path.join(space_time_path, 'evaluatespacetime.h5'), 'w')
